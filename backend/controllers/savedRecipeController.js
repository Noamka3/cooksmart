...
const normalizeRecipePayload = (payload = {}) => {
  ...
  const bonusRecipes = Array.isArray(payload.bonusRecipes)
    ? payload.bonusRecipes
        .filter((item) => item?.title && item?.content)
        .map((item) => ({
          title: item.title.trim(),
          missingIngredients: Array.isArray(item.missingIngredients)
            ? item.missingIngredients.filter((value) => typeof value === "string" && value.trim())
            : [],
          content: item.content.trim(),
        }))
    : [];

  return { title, ingredients, instructions, notes, bonusRecipes };
};
